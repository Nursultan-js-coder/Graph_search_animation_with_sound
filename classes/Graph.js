...
class Graph {
  ...
  addEdge(v, w, weight, xv, yv, xw, yw) {
    ...
    if (this.adj[w].head === null) {
      ...
      this.adj[v].addLast(
        new Node(w, weight, this.adj[w].head.x, this.adj[w].head.y)
      );
      this.data[v][w] = weight;
    }
    if (this.adj[v].head === null) {
      ...
      this.data[w][v] = weight;
      this.adj[w].addLast(
        new Node(v, weight, this.adj[v].head.x, this.adj[v].head.y)
      );
    }
  }
  ...
  //prints all path from src to destination
  ...
  async printResultPath(path, weight, isShortest) {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      node.colorify(this.song);
      fill(255);
      noStroke();
      textSize(20);
      strokeWeight(4);
      if (isShortest) fill("red");
    ...
    this.x += 30;
    if (isShortest) text("shortest", this.x, this.y);
    ...
    if (isShortest) {
      ...
      for (let i = 1; i < path.length; i++) {
        ...
        stroke("red");
        line(
          this.adj[path[i - 1]].head.x,
          this.adj[path[i - 1]].head.y,
          this.adj[path[i]].head.x,
          this.adj[path[i]].head.y
        );
        this.adj[path[i - 1]].head.colorify(this.song);
      }
      last.colorify(this.song);
    }
    this.y += 20;
    this.s;
  }
